...
do
  ...
  -- Match the existing Home Manager Neovim settings.
  vim.o.expandtab = true
  vim.o.shiftwidth = 2
  -- Relative line numbers make jump motions easier to judge.

  -- Keep mouse capture disabled so terminal shortcuts and Herdr keep working.
  vim.o.mouse = ''
  ...
  -- Keep invisible whitespace hidden by default. Use <leader>tl when auditing it.
  -- `vim.opt` updates the global-local option for the current window too, which
  -- matters when this file is sourced in an already-open buffer.
  vim.opt.list = false
...
do
  ...
  --  See `:help hlsearch`
  -- Escape keeps the existing save behavior while clearing search highlights.
  ...
  vim.keymap.set('n', '<leader>tl', '<cmd>setlocal list!<CR>', { desc = 'Toggle visible whitespace' })
...
do
  ...
  -- Keep the lightweight picker and file-browser workflow from the old config.
  vim.pack.add {
...
-- ============================================================
-- SECTION 6: LSP
-- LSP keymaps, server configuration, Nix-managed tools
-- ============================================================
do
  -- [[ LSP Configuration ]]
  ...
  --  - and more!
  --
  -- Language servers and formatters are installed declaratively by Home Manager
  -- and exposed on PATH, so editor state stays separate from the Nix toolchain.
  --
  ...
  -- Enable the following language servers. Their executables are installed by Nix.
  -- Feel free to add or remove servers as your toolchain changes.
  ...
  -- The language servers are installed declaratively through Home Manager.
  ...
  -- Command-line launches can set a buffer's filetype before init.lua runs.
  -- Replay the event so the current buffer gets the same LSP behavior as a
  -- buffer opened after startup.
  if vim.bo.filetype ~= '' then vim.schedule(function() vim.cmd.doautoall 'nvim.lsp.enable FileType' end) end
end
